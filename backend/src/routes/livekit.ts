...
import { createIngress, createStreamerToken, createViewerToken, resetIngresses } from "../utils/livekit/livekit";
...
router.get("/viewer-token",verifyToken, async (req: Request, res: Response) => {
    ...
    try {
        const token = await createViewerToken(roomName as string, identity as string);
        res.status(200).json({token});
...
router.get("/create-ingress", verifyToken, async (req: Request, res: Response) => {
    ...
    try {
        const ingress = await createIngress(roomName as string, ingressType as unknown as IngressInput);
