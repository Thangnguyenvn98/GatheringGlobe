...
const categories = ['nature', 'city', 'tech', 'people', 'animals', 'food', 'sports'];
...
export const generateEvents = async (numEvents:number, users:any[]) => {
    ...
    for (let i = 0; i < numEvents; i++) {
      ...
      const daysFuture = faker.number.int({ min: 1, max: 7 }); 
      const startTime = new Date(refDate.getTime() + daysFuture * 24 * 60 * 60 * 1000); 
      const endTime = new Date(startTime.getTime() + faker.number.int({ min: 3600000, max: 86400000 })); 
      const event = new Event({
        ...
        category: faker.helpers.arrayElement(['Music', 'Art', 'Technology', 'Sports']),
        eventType: faker.helpers.arrayElement(['Concert', 'Exhibition', 'Conference', 'Match']),
